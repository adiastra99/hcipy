...
def least_inv(M, inv_method='tikhonov', num_modes=None):
	'''Given a matrix M such that Mx = b, makes a least-squares inversion matrix
	N such that x^ = Nb, where x^ is the least-squares approximation to x.

	Parameters
	----------
	M : ndarray
		The matrix from which to make a least-squares approximation.

	Returns
	-------
	N : ndarray
		The least-squares approximated matrix.
	'''
	MT = M.T
	inverted = inverse_tikhonov(MT.dot(M))
	N = inverted.dot(MT)
	return N
